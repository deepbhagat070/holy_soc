    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (clr) begin
                mem_RegWrite <= 1'b0; mem_MemtoReg <= 3'b0; mem_MemRead <= 1'b0;
                mem_MemWrite <= 1'b0; mem_funct3 <= 3'b0; mem_alu_result <= 32'b0;
                mem_rs2_data <= 32'b0; mem_pc_plus_4 <= 32'b0; mem_pc <= 32'b0; 
                mem_imm_ext <= 32'b0; mem_branch_target <= 32'b0; mem_is_ecall <= 1'b0; 
                mem_is_mret <= 1'b0; mem_is_ebreak <= 1'b0; mem_csr_we <= 1'b0; 
                mem_csr_addr <= 12'b0; mem_csr_wdata <= 32'b0; mem_csr_rdata <= 32'b0;
                
                // CRITICAL: Zero architectural address
                mem_rd_addr <= 5'b0; 
            end else begin
